...
public static class ConfigurationHelper
{
    ...
    public static Option<bool, Error> LoadEnvironmentVariables(IEnvironment environment, string configFilePath, IEnumerable<string> envFiles)
    {
        ...
        var configFileEnv = Path.Combine(configFileDirectoryName, Constants.Default.DotEnvFileName);
        if (environment.FileExists(configFileEnv))
        {
            Env.Load(configFileEnv);
        }
        // .env.local file next to a dbup.yml
        var configFileEnvLocal = Path.Combine(configFileDirectoryName, Constants.Default.DotEnvLocalFileName);
